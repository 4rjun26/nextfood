...
const profile = ({posts,selects}) => {
  const router = useRouter()
    ...
    function showoptions(){
        const elem=document.getElementById("optionboard");
        const dc=document.getElementById("downchev");
        if(elem.style.height=="40px"){
            elem.style.height="300px";
            dc.style.transform="rotate(-180deg)";
            dc.style.marginTop="250px";
        }
        else{
            elem.style.height="40px";
            dc.style.transform="rotate(360deg)";
  ...
  return (
    <div style={{"margin-top":"60px"}}>
        ...
        <div className={styles2.optionboard} id="optionboard">
            <button className={styles2.downchev} id="downchev" onClick={showoptions}><FaChevronCircleDown/></button>
        </div>
        {selects.map((select) => (
